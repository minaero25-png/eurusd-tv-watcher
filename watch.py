...
def analyze_flip(changes: list, fj_items: list) -> str:
    ...
    prompt = (
        ...
        "Be honest about the causal link:\n"
        "- If the headline reaches EUR/USD only INDIRECTLY (via oil, via a third country like "
        "Russia/China, or through several steps), SAY SO plainly in reasoning_th and keep "
        "severity + credibility low — never dress up an indirect link as a confident driver.\n"
        "- Do NOT state outdated textbook relationships as fact. In particular: higher oil no "
        "longer reliably weakens the USD — the US is a net energy exporter now, and oil or "
        "Middle-East risk shocks often STRENGTHEN the USD as a safe haven while hurting energy-"
        "importing Europe (EUR). If the direction is genuinely contested, say it could go either "
        "way and why, instead of forcing one side.\n"
        "If you do pick a driver, rate:\n"
        "- severity: how market-moving for EUR/USD — HIGH (>30 pips potential) / MEDIUM (10-30) "
        "/ LOW (minor, or only an indirect link)\n"
        "- credibility: how reliable this is as THE driver — HIGH (confirmed news + a direct, "
        "well-established EUR/USD link) / MEDIUM (real news but indirect or debated link) / "
        "LOW (unconfirmed, or the link to EUR/USD is weak or contested)\n\n"
