...
def analyze_flip_with_groq(changes: list, fj_items: list) -> str:
    ...
    prompt = (
        ...
        f"{headlines}\n\n"
        "Decide whether any headline is a PLAUSIBLE driver of THIS EUR/USD move. Be strict:\n"
        "- RELEVANT: US or Euro-area data (CPI/NFP/PMI/GDP/retail/jobless), Fed/ECB speakers or "
        "policy, USD/EUR/DXY/US-Treasury yields, oil, or geopolitics that clearly moves the "
        "dollar or euro.\n"
        "- NOT relevant — set driver_th=null EVEN IF the time matches: China domestic flows, "
        "single-stock or sector news, equity indices, crypto (unless a broad risk-off), "
        "Asia-only or EM-only stories, anything with no clear EUR or USD transmission.\n"
        "- Prefer a scheduled data release or official central-bank action over loose commentary.\n"
        "If NO headline clears this bar, set driver_th=null. Most TA flips are price action / "
        "technical — a null answer is CORRECT and expected. Do NOT force a weak link just because "
        "a headline is near in time.\n"
